...
export const generateIdeas = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: "Failed to generate ideas", error: error.message });
    }
};
...
export const getUserStats = async (req, res) => {
    ...
    try {
        ...
        const sprouting = await Idea.countDocuments({ userId, currentStage: { $gte: 1 } });
        const blooming = await Idea.countDocuments({ userId, currentStage: { $gte: 3 } });
